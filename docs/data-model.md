...
| [Italy Quiz](italy-quiz.md) | `italy.tour.{placeId}`, `italy.progress.v1`, `italy.help.seen.v1` | Per-place tour in progress (drawn order, position, missed ids); per-place best stars, best percentage, play count and seen-question set; first-run help flag |

Two design choices recur across them. Progress is stored as a **run position** (Flags Quiz, Coat of Arms, Italy Quiz) or a **solved-id set** (Logo Quiz, Sport Quiz), never as derived state — levels, counts, and completion are recomputed from the primitive so a content update cannot invalidate saved progress. Italy Quiz persists its tour's drawn ORDER alongside the position, because that order is random: a place holds more questions than a tour takes, and recomputing the draw on a re-render would reshuffle a tour under a player halfway through one. A saved order whose ids no longer all exist is discarded for a fresh draw. And nothing here is `quiz.*`-prefixed, so the erudite settings reset leaves every sibling app's progress untouched.
