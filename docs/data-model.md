...
The descriptor also carries a `template` key — the backend's App Template code, naming the root experience the app belongs to. No client reads it today; each build still picks its experience from the build-time slug. It is served so a future build can be told what it is by the backend rather than by its own binary, which is the direction the [configurable template](configurable-template.md) points in.
...
| [Italy Quiz](italy-quiz.md) | `italy.tour.{placeId}.c{n}`, `italy.progress.v2`, `italy.help.seen.v2` | Per-circle tour in progress (fixed ids, position, missed ids); per-place list of circles, each with its frozen question set, best stars, best percentage and play count; first-run help flag |

Two design choices recur across them. Progress is stored as a **run position** (Flags Quiz, Coat of Arms, Italy Quiz) or a **solved-id set** (Logo Quiz, Sport Quiz), never as derived state — levels, counts, and completion are recomputed from the primitive so a content update cannot invalidate saved progress. Italy Quiz persists the ids of the circle being played alongside the position, and a save only resumes when those ids are the same set in the same order as the circle being entered. That is what stops a half-finished tour from resurrecting under a different twenty questions. Circle sets themselves are a durable part of the record, not a cache: a circle is drawn once and then frozen, so a replay is the same exam — see [Italy Quiz](italy-quiz.md#circles-ten-fixed-sets-per-place). The `italy.progress.v1` key it grew out of is migrated into circle 1 and then left in place, so an app rollback still finds the player's stars. And nothing here is `quiz.*`-prefixed, so the erudite settings reset leaves every sibling app's progress untouched.
...
| `GET /apps/{slug}/theme` | The operator-authored colour tokens for a configurable-template build |
...
The theme endpoint is the odd one out: it takes **no query parameters** (colours are locale-independent) and answers a conditional `GET` with a **strong ETag** hashed over the emitted bytes, so the steady state is an empty-bodied `304` rather than a payload. It returns `{ schema_version, onboarding_type?, theme: { name, supports_dark, light, dark } }`, where each appearance map carries all forty-five remote colour tokens (`schema_version: 2` as of the Э1 widening; `onboarding_type` is a sibling of `theme`, carried only for the configurable template). That key takes exactly three values — `classic`, `universal`, or `none` — and the client union in `lib/onboarding/onboarding-type.ts` must equal the backend's admin enum in order, since nothing can check the two repositories against each other. An absent or unrecognized value resolves to the default variant instead of rejecting the palette; `none` means no intro is shown at all. Only a build listed in `T_TEMPLATE_SLUGS` ever calls it — see [Configurable Template](configurable-template.md). The same `theme` object also rides along on `/snapshot` (as a `theme` key beside `theme_schema_version`) when an operator has actually overridden something; the client parser is shared, but that second transport is not wired yet. A theme edit deliberately does **not** move the snapshot's `version` — the ETag is the theme's own, much cheaper validator.
...
- [Configurable Template](configurable-template.md) -- The theme payload and its on-device record
