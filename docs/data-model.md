...
| [Italy Quiz](italy-quiz.md) | `italy.tour.{placeId}.c{n}`, `italy.progress.v2`, `italy.help.seen.v3` | Per-circle tour in progress (fixed ids, position, missed ids); per-place list of circles, each with its frozen question set, best stars, best percentage and play count; first-run help flag |
