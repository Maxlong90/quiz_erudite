...
The route above never leaves this machine. `eas build` does: it packages the project and uploads it, and the root `.easignore` is what decides the contents. The one thing to know before touching that file is that **`.easignore` replaces `.gitignore` rather than adding to it**. From the moment it exists eas-cli reads no `.gitignore` anywhere in the tree; only `.git` and `node_modules` stay hardcoded ([EAS docs](https://docs.expo.dev/build-reference/easignore/), which also prescribe the remedy: "include all files and directories from your .gitignore file and add additional files you want to ignore"). So the obvious edit — a `.easignore` holding just the rule you wanted to add — silently stops applying everything `.gitignore` was hiding: `.env`, `android/` (2.9 GB of local prebuild output), `dist/`, `.expo/`, and the `*.jks` / `*.p8` / `*.p12` / `*.mobileprovision` patterns that exist to keep signing material off other people's machines.

How bad that is depends on which VCS client runs, and it is worth knowing which one you are on. By default eas-cli uses its **`GitClient`**: the archive is a `git clone --depth 1` of the *tracked* tree, and `.easignore` is applied afterwards by deleting whatever `git ls-files --exclude-from .easignore --ignored --cached` reports. An untracked `.env` is never in that clone, so the naive one-liner cannot leak it on this path. Under **`EAS_NO_VCS=1`** (or a non-git checkout) eas-cli instead walks the **working copy** filtered by `.easignore` alone — and there it really would upload `.env`, `android/` and `dist/`. The rules below are what stands between that path and a leak.
...
Two guards cover this, and they cover different builds — the same division of labour as `SplashConfigGuard` and its mirror above. `App\Services\Build\EasArchiveGuard` fails a **pipeline** build in seconds, naming the file, the rule and its line number. A developer's local `eas build` gets no such tripwire, so `npm run check:eas-upload` (`__tests__/lib/easignore.test.ts`) is the mirror on this side: it runs the committed rules through `ignore` — the same matcher eas-cli uses — and asserts every file the build needs survives, every pack source does not, and `.easignore` is still `.gitignore` plus exactly one rule. One of its assertions is not a model of anything. It runs the `GitClient` path's own step, `git ls-files --exclude-from .easignore --ignored --cached`, and pins the exact list of files the clone loses: the twelve pack sources and one stale `.playwright-mcp` log that a copied `.gitignore` rule already matched. If any other file ever joins that list, the rules changed something nobody intended.

Note that `eas update` is unaffected by any of this. An OTA payload is Metro's output, not a working copy, so `.easignore` cannot make one smaller.
...
Runs the Jest suite (`jest-expo` preset). The test files live in `__tests__/` and cover the device-local business logic in `lib/` and `hooks/` — content-cache namespacing and the two-variant image collection, the hint and lives economies, answer stats, store links, RevenueCat gating (including the per-slug committed-key scoping), the fail-closed purchase policy for every app's shop, the Logo Quiz and Flags Quiz content transforms, and similar pure logic. `__tests__/app/` also holds screen-level integration tests that render a screen with its dependencies mocked: one pins the API-fallback no-repeat guarantees (dedupe by ID, seen filter), another pins the Coat of Arms reveal (the original image appears only after a correct answer, never when the question has none, and always at exactly the base coat's geometry), another pins the Sport Quiz shop's money invariant (coins are credited only on a resolved purchase — never on a cancellation or a store failure), and `__tests__/app/italy-quiz-circles.test.tsx` drives the Italy Quiz map and quiz screens through a whole circle — a fresh player clears Rome's circle 1 on ten of twenty, Florence opens, re-entering circle 1 serves the same twenty in the same order, a mistakes review moves no star, and circle 2 reads "soon" because Rome's content is one circle deep (see [Italy Quiz](italy-quiz.md#circles-ten-fixed-sets-per-place)). The remote theme engine is covered end to end — wire parsing, cache records, the conditional fetch, the palette overlay, the template's derived-token funnel, its inertness on every other build, and the no-colour-literal scan over the template surface and its imports. Artwork the *build* consumes is covered *structurally* rather than by rendering, because jest-expo rewrites every image module to a number and no rendering test in this repo can tell which PNG a `require()` resolved to: `__tests__/app/t-asset-packs.test.ts` checks every asset-pack slot against the size its manifest declares, and `__tests__/app/splash-native-config.test.ts` checks the native splash entry in `app.json` (see [The splash entry in `app.json` must always name an image](#the-splash-entry-in-appjson-must-always-name-an-image)). Both open the bytes through the shared `__tests__/helpers/png.ts`, which proves a file really is a PNG before reading its dimensions out of the IHDR chunk — a truncated write or a JPEG renamed to `.png` then fails here as a named assertion instead of inside `@expo/image-utils` during prebuild. The helper sits under `__tests__/helpers/` so Jest's `testMatch` glob does not collect it as a suite with no tests — as do `png.ts`'s neighbours `repo-tree.ts` (the shared filesystem walk), `asset-packs.ts` (the manifest readers those suites derive their expectations from rather than transcribing them) and `eas-upload.ts` (the upload filter described below). Screen *layout* is pinned the same way, and for the same reason: a claim like "no phone changed" is worth nothing as prose. `__tests__/hooks/use-responsive.test.ts` walks every iPhone window size through the responsive gate, and `__tests__/lib/coat-of-arms-layout.test.ts` asserts the Coat of Arms gameplay metrics still equal the constants that shipped on tall phones while proving the answer-grid height fit shrinks the coat (never below its floor) on the short and narrow windows that used to clip the buttons — so a future scaling tweak that reaches a tall phone, or a broken fit that clips answers, fails here rather than in App Review (see [Laying out for a window that can change size](coat-of-arms-quiz.md#laying-out-for-a-window-that-can-change-size)). Tests that need a specific window size set it through `__tests__/helpers/window.ts`, another helper kept out of `testMatch`'s way. The same structural reasoning covers what the build *uploads*: `__tests__/lib/easignore.test.ts` runs the committed `.easignore` through `ignore`, the matcher eas-cli itself uses, and asserts that the pack sources are dropped while every file the backend injects under `assets/` survives (see [What `eas build` uploads](#what-eas-build-uploads-and-why-easignore-is-a-copy-of-gitignore)). There is no device, emulator, or backend dependency, so the ninety-seven files (1943 tests) finish in **well under a minute** — 12 seconds measured on the dev host — and are safe to run on every change. It is not a long-running operation.
...
App Review tests an iPhone-only build on an iPad, where iPadOS 26 runs it in a resizable window — and that is how [Coat of Arms](coat-of-arms-quiz.md#laying-out-for-a-window-that-can-change-size) collected a Guideline 4 rejection. This host is Linux, with no macOS and no iPad simulator, so the layout is checked in a browser instead.

That substitution is faithful rather than approximate. `react-native-web` implements `useWindowDimensions` on top of `window.innerWidth` and the browser `resize` event, so changing the viewport delivers the same `Dimensions.change` → re-render the app sees on a real device, with no remount. A layout bug that depends on a *native* measurement would escape this, but a layout computed in JS cannot.

Run it in two steps. Start the web dev server for the slug you are checking, detached — a dev server never exits, so foregrounding it strands the turn:
...
The script launches headless Chrome, talks to it over the **Chrome DevTools Protocol (CDP)**, and writes PNGs into `screenshots/` (git-ignored), printing each filename as it goes. `label` prefixes the files — use `before` and `after` around a change. `mode` selects the work:

| `mode` | What it captures |
|--------|------------------|
| `sweep` | Every screen at eleven window sizes, from 320 × 568 to 1024 × 768 — including the short and narrow ones (360 × 610, 500 × 680, 700 × 560, 1024 × 700) the answer-grid height fit exists for; 360 × 610 is where the bottom answer row used to clip off screen |
| `identity` | The two gameplay screens at the three phone widths, with the question order pinned |
| `reveal` | The post-answer reveal, where a broken content column drops the "Next" button |
| `modals` | The report and help sheets, which are portals outside the content column |
| `resize` | One live page resized without reloading — the direct test of the original bug |
| `all` | All of the above (the default) |

Two of those modes exist for reasons that are not obvious. **`identity`** is what answers "did phones change?": it pins the question order through the `retry` parameter, which `useRunProgress` replays verbatim and never persists, so a `before` and an `after` run draw the same countries and the PNGs are comparable. Without it every gameplay shot shows a different country and each diff is noise. **`resize`** is the only mode that exercises the bug the rejection was really about, since a layout frozen at import time looks correct until the window changes.

Reading the results: phone widths (320–440) must match between `before` and `after`. Allow for two honest sources of difference — a coat image still streaming in, and the shuffled question order outside `identity` mode. Compare layout, not content.

A full `all` run takes roughly ten minutes, most of it deliberate waiting: gameplay screens fetch and cache the content snapshot before first paint, and under-waiting yields half-painted screenshots that read as layout bugs. It prints a line per screenshot, so it is safe in the foreground — see [Long-Running Operations](long-running-operations.md#the-web-dev-server-behind-the-responsive-sweep). Load one route in the browser yourself first if the server is cold, so the initial web bundle is not built inside the script's own timeouts.
