...
The `logo-quiz-preview` / `logo-quiz-production` and `sport-quiz-preview` / `sport-quiz-production` profiles in `eas.json` set `EXPO_PUBLIC_APP_SLUG` plus that app's bundle id, package, and RevenueCat keys. Those values ship as `REPLACE_WITH_…` placeholders; until an operator fills them the identity vars fall back to the Erudite identity and the placeholder keys are rejected by the key shape check, so the sibling's store products do not resolve and its shop fails closed on a device (local-granting only in Expo Go). Filling the placeholders is a separate ops step, not a code change — for [Sport Quiz](sport-quiz.md#coin-packs) it is the last thing standing between its wired-up coin packs and real iOS revenue. The Sport Quiz profiles deliberately carry **no** `EXPO_PUBLIC_REVENUECAT_ANDROID_KEY` (that app has no Google Play catalog, so Android must stay fail-closed) and no AdMob unit id (it shows no ads). Flags Quiz and Coat of Arms still have no dedicated profile, so their bundle id and package fall back to the Erudite identity; that is harmless, since they ship no shop or paywall.
