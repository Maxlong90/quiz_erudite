...
**Schema version** — The one number both halves of the theme contract agree on: the `schema_version` the theme endpoint serves and the `CLIENT_THEME_SCHEMA_VERSION` this build declares in `lib/theme/contract.ts` (both `2` today). It bumps only for a change that would make an old client render something *wrong* — a renamed or removed token, or a changed value domain — never for a purely additive key, because store-review latency means the client cannot ship first. A client that meets a newer version rejects the whole envelope as `unsupported-schema`, keeps its last-known-good palette, and warns once per launch; an offline parity test and an opt-in live check pin the two sides to the same number. See [Configurable Template](configurable-template.md#sibling-keys-and-when-the-version-bumps).
