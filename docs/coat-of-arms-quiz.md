...
Russian is the one locale that overrides the backend prompt: `constants/coat-of-arms/labels.ts` supplies its own `quizPrompt` ("Какой стране принадлежит этот герб?"), and every other locale keeps the backend question. Whichever wording applies, the prompt is a single ordinary string that wraps **naturally** within the two-line cap the layout enforces — it carries no hard `\n`. An early version hard-wrapped the Russian override onto two lines with an embedded newline, which on iOS made `adjustsFontSizeToFit` silently drop the second line, so the prompt clipped to just "Какой стране". The fix removed the newline.

The two-line cap still has to hold, because `coatQuizMetrics` sizes the coat frame from a fixed two-line prompt allowance (see [The answer grid is the anchor](#the-answer-grid-is-the-anchor)). So the prompt can neither overflow two lines nor lose a line to a flaky auto-shrink. Rather than trust `adjustsFontSizeToFit` — which `react-native-web` ignores entirely, clipping long prompts, and which misbehaved on iOS — `quiz.tsx` computes the font size **deterministically** with `fitPromptFontSize`: it shrinks the size until the whole string word-wraps inside two lines of the content column, padding Cyrillic wider than Latin because it renders wider. The result is always at or below the metric-supplied `promptFont`, so the two-line height bound (and thus `coatSize`) is unchanged. This mirrors the `fitFontSize` approach already used for the answer-button labels.
