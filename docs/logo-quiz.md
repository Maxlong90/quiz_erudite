...
To keep the splash both useful and non-blocking, the preload is split. The first `PRELOAD_MIN_LOGOS` (45 — three levels of 15) are awaited so navigation can gate on them; the rest are prefetched in the background so a large catalog never delays the first screen. The whole routine is fail-open: any sync or prefetch error resolves quietly and never blocks the splash.

Navigation is gated on the outcome. A Logo Quiz build leaves the splash only after **both** the `SPLASH_DURATION_MS` minimum (3 seconds, versus 2500ms for other builds) **and** the first-levels preload complete — raced against a hard cap of `SPLASH_HARD_CAP_MS` (10 seconds) so a slow network or huge catalog can never trap the user on the splash. Non-Logo-Quiz builds keep the simple fixed-duration timer unchanged. This preload is separate from `app/logo-quiz/index.tsx`, which loads the bundled UI art (`WELCOME_ASSETS`) — that warms local artwork, while this warms the remote brand logos.
