...
A correct answer (or a skip-hint reveal) plays an in-place animation rather than navigating away. The wrong options fade out (~1s) while the correct green answer simultaneously glides up to sit centered under the question (~1.7s), driven by `react-native-reanimated`. Once it lands, the screen swaps the grid for a settled layout: the centered green answer, then an Explanation panel showing the question's localized `explanation` (blank/null explanations are omitted), and a `Next` button above the hint row.
