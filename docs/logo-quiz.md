...
**Share a logo** invokes the native share sheet via React Native's `Share`. The message is a localized invite to guess the logo with a store link appended — the link comes from `getStoreLinks(snapshot.app)` for the current platform, so it always points at the correct listing (see [Content and Offline](content-and-offline.md#store-links-from-the-app-config)). Order matters here: `handleShare` presents the share sheet **before** closing the menu. Closing first unmounted the RN `Modal` while iOS was still trying to present the system sheet, which dropped the presentation so nothing appeared — this was why share was broken. The fix mirrors the working erudite `ShareQuestionButton` (share, then close in a `finally`). A cancelled or platform-rejected share is a silent no-op.
