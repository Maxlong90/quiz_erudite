...
`app/onboarding.tsx` decides the destination after `markSeen` runs (both on the final "Start" press and on "Skip"). The gate is capability-driven, not platform-hardcoded: it shows the paywall only when **all three** conditions hold — this is the **erudite build** (`currentTemplate() === null`), store billing is actually enabled on this platform (`revenueCatEnabled`), **and** the per-platform backend flag is true (iOS reads `show_paywall_ios`, Android reads `show_paywall_android`).

The build check is defence in depth. This paywall sells *erudite* premium, and a sibling app never reaches this screen anyway — `app/index.tsx` redirects it to its own splash first. But that redirect is a single point of failure, and this exact class of mistake has shipped before: an unregistered template silently inherited the erudite intro on Logo Quiz (the reason `__tests__/app/app-templates.test.tsx` exists). The live motivator is Sport Quiz, a coins-only app with no premium tier, no entitlement and no offering, whose backend config nevertheless returns `show_paywall_ios: true` — and which now has a configured iOS RevenueCat key, so `revenueCatEnabled` is true for it too. Both of the original conditions are satisfiable for an app with nothing to sell, so the build check is what keeps the pitch away. Gating on `revenueCatEnabled` guarantees a paywall is never forced on a platform that cannot charge, so iOS stays safe until its RevenueCat key is supplied and then lights up automatically with no code change. Any other case — a missing or not-yet-loaded snapshot, the flag absent or false, web, or a disabled store — sends the player straight to home. The default is therefore "no forced paywall", which is the reviewer-safe state.
...
The grant now splits by whether the platform can actually charge the player. On a real store platform — `Platform.OS` is `android` or `ios` — the purchase must go through RevenueCat, and if the store is unavailable or a product id is missing, the purchase fails closed: it throws so the UI shows a purchase error and nothing is granted. The local stub grant runs only in genuinely non-store dev environments, gated explicitly on `isExpoGo || Platform.OS === 'web'`, never on iOS or Android. `isExpoGo` is exported from `lib/revenuecat.ts` precisely so the two cases can be told apart.

That policy is written down once, in `lib/store-purchase.ts`, and every app's shop buys through it — Erudite's bundles, Logo Quiz's coin and life packs, and Sport Quiz's coin packs. Each app keeps only its own catalog and its own economy mutation; a shop cannot opt out of the fail-closed rule, because the rule *is* the purchase call.
...
RevenueCat is enabled per-platform by **capability, not by a hardcoded `Platform.OS`**: the SDK turns on for any native platform that has a public store key configured. Android ships with a committed key for the erudite build, so that build is always on; iOS has no committed key for any app, so it stays disabled until the owner supplies `EXPO_PUBLIC_REVENUECAT_IOS_KEY`, then lights up automatically with no code change (see [iOS Monetization Parity](ios-monetization-parity.md)). RevenueCat is therefore disabled today in Expo Go, on web, on iOS, or whenever the native module is missing. What that disabled state does depends on whether the platform can charge the player. In Expo Go and on web — genuine dev environments — the shop and paywall still fall back to the local-grant stub so the dev flow never breaks. On a real iOS device, **both** the shop's consumable purchases and the paywall's premium unlock now fail closed with an error rather than local-granting — the parity work brought the paywall in line with the fail-closed consumable policy so no free premium is ever handed out on a real store device. The public Android key is read from `EXPO_PUBLIC_REVENUECAT_ANDROID_KEY`, falling back to the committed key **of the app being built** (a per-slug table in `lib/revenuecat.ts`); release builds set it explicitly per profile in `eas.json` (`preview` and `production`). Every wired key must belong to the RevenueCat project the backend provisions *for that app* — a mismatch makes `getOfferings` return an empty `default` offering, which is exactly the all-null-packages condition the free-unlock guard now blocks. Scoping the committed key per slug is how that requirement is enforced rather than merely documented: a sibling can no longer inherit Erudite's key by accident, and a key that does not even look like one (an unfilled `REPLACE_WITH_…` placeholder) is treated as absent. See [Development](development.md#configure-the-backend) for the key wiring.
