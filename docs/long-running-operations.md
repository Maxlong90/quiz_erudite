...
The Jest suite is worth calling out precisely so nobody defensively backgrounds it. All 70 test files are pure logic and mocked-dependency screen tests with no device, emulator, or backend involved, and the whole run finishes in **seconds, not minutes** — around 12 on a warm cache, under 30 cold. Run `npm test` in the foreground.
