...
The Jest suite is worth calling out precisely so nobody defensively backgrounds it. All 72 test files (875 tests) are pure logic, filesystem checks, and mocked-dependency screen tests with no device, emulator, or backend involved, and the whole run finishes in **seconds, not minutes** — 8 seconds measured on a warm cache, under 30 cold. Run `npm test` in the foreground.
