/**
 * The configurable template's results screen — a deliberate COPY of
 * app/results.tsx, for the reasons app/t/quiz.tsx's docblock sets out: a screen
 * owns a route, so each app gets its own; the leaf components it renders
 * (AchievementUnlockModal, ScreenBackground) are shared, not duplicated.
 *
 * The diff against app/results.tsx is four things:
 *  1. the palette arrives through hooks/t/use-template-theme.ts;
 *  2. the traffic-light score scale reads the funnel's derived tier roles
 *     instead of three hardcoded hexes (see the scoreColor site below);
 *  3. "Play again" relaunches '/t/quiz' rather than '/quiz';
 *  4. "Home" lands on '/t' rather than '/', which is the bug this port fixes —
 *     on a template build app/index.tsx redirects '/' to '/t/splash', so every
 *     finished run used to bounce the player through the splash screen.
 *
 * makeStyles keeps its EruditePalette signature: a TemplateTheme already
 * satisfies it, and the tier colour is applied inline in JSX rather than in the
 * stylesheet, so no style needs the wider type.
 */
...
export default function ResultsScreen() {
  ...
  // The shipped scale is a hardcoded green/amber/red. tierMid is the brand
  // accent rather than an amber — an intentional pixel change whose reasoning
  // lives in hooks/t/use-template-theme.ts, and the reason the whole ring
  // repaints with an operator preset.
  const scoreColor =
    tier === 'excellent' ? colors.tierHigh : tier === 'good' ? colors.tierMid : colors.tierLow;
