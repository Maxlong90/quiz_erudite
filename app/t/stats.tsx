/**
 * The configurable template's stats screen — a deliberate COPY of app/stats.tsx,
 * for the reasons app/t/quiz.tsx's docblock sets out: a screen owns a route, so
 * each app gets its own; the leaf components it renders (ScreenBackground,
 * AchievementRow, BottomBar) are shared, not duplicated.
 *
 * The diff against app/stats.tsx is two things:
 *  1. the palette arrives through hooks/t/use-template-theme.ts — at ALL THREE
 *     call sites, the screen, Row and Divider;
 *  2. the accuracy row's traffic-light scale reads the funnel's derived tier
 *     roles instead of three hardcoded hexes. That makes this the SECOND consumer
 *     of the tier scale (app/t/results.tsx was the first), which is what the
 *     funnel hook's "a derived role earns a name only if TWO OR MORE screens use
 *     it" rule requires of a named derived role — until now that rule described
 *     an intention rather than a fact. tierMid is the brand accent rather than an
 ...
 * prop rather than in the stylesheet, so no style needs the wider type.
 */
        ...
        {isEmpty ? (
          ...
          >
            ...
            <View style={styles.card}>
              ...
              <Divider />
              {/* The shipped scale is a hardcoded green/amber/red. tierMid is the
                  brand accent rather than an amber — an intentional pixel change
                  whose reasoning lives in hooks/t/use-template-theme.ts, and the
                  reason this row repaints with an operator preset. */}
              <Row label={t('stats.accuracy')} value={`${accuracy}%`} accent={accuracy >= 70 ? colors.tierHigh : accuracy >= 40 ? colors.tierMid : colors.tierLow} />
              <Divider />
