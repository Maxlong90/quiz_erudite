/**
 * The configurable template's quiz screen — a deliberate COPY of app/quiz.tsx.
 *
 * WHY A COPY RATHER THAN A SHARED COMPONENT
 * -----------------------------------------
 * The line this repo already draws (docs/configurable-template.md, and the six
 * sibling app/<slug>/quiz.tsx files that predate this one): a SCREEN owns a
 * route, so each app gets its own; a LEAF COMPONENT owns none, so every app
 * shares one. Everything this file renders — QuestionCard, LivesBar, HintBar,
 * the modals — is imported from components/, not duplicated. What is duplicated
 * is exactly the part that differs: which routes this screen navigates to.
 *
 * Extracting app/quiz.tsx into a prefix-parameterised component would be a large
 * refactor of shipped, untested-by-this-task code that four live Erudite suites
 * pin, to serve a screen that is SUPPOSED to diverge — the template's modes and
 * economy become operator data in later work.
 *
 * THE DIFF AGAINST app/quiz.tsx IS DELIBERATELY TINY
 * -------------------------------------------------
 * Five things, and nothing else:
 *  1. the palette arrives through hooks/t/use-template-theme.ts (the funnel every
 *     app/t screen reads through — enforced by t-no-color-literals.test.ts), at
 *     both call sites: QuizScreen and TotalTimer;
 *  2-5. the five route strings are /t-prefixed — '/t/results' on completion and
 *     '/t' on the four ways out (close, the two out-of-lives gates, the error
 *     state). On a template build app/index.tsx redirects '/' to '/t/splash', so
 *     a missed one bounces a finished player through the splash screen. That is
 *     the bug this port exists to fix; __tests__/app/t-routes.test.ts now fails
 *     on any route literal under app/t/ that is not /t-prefixed.
 *
 * Keep it that way. Behaviour changes belong in app/quiz.tsx first, so the two
 * stay diffable; makeStyles/makeTotalTimerStyles keep their EruditePalette
 * signature because a TemplateTheme already satisfies it.
 */
    ...
    return (
      <ScreenBackground>
        ...
        <OutOfLivesModal
          ...
          onClose={() => { setOutOfLivesOpen(false); router.replace('/t'); }}
          onWatchAd={async () => {
    ...
    return (
      <ScreenBackground>
        <SafeAreaView style={styles.centered}>
          ...
          <Pressable onPress={() => router.replace('/t')} style={styles.homeLink}>
